...
DOM — это объектная модель документа, которая отражает структуру HTML в виде дерева.  
Каждый HTML-элемент становится отдельным узлом (**node**) в этом дереве.  
...
Render Tree — это промежуточное представление страницы, которое оптимизировано именно для отрисовки.  
По нему браузер будет считать расположение элементов и рисовать их.
...
**Reflow** — это процесс перерасчёта расположения и размеров элементов на странице.  
Он запускается, когда меняется структура документа или свойства, влияющие на геометрию, например:
